...
export class IpcMain {
  ...
  setup() {
    ...
    ipcMain.on('enableWalletZemu', async (event, arg) => {
      let ret = {};
      try {
        let index = arg.index;
        const info = await this.provider_zemu.enable(index);
        let accountInfo = info[0];
        let accountPubKey = info[1].toUint8Array();
        ret = {
          success: true,
    ...
    ipcMain.on('signMessageZemu', async (event, arg) => {
      let argBuffer = Buffer.from(arg);
      let signature = await this.provider_zemu.sign(Bytes.fromBuffer(argBuffer));
      let signatureArray = signature.toUint8Array();
      let ret = { signed: signatureArray, original: arg, label: 'signMessage reply' };
      event.returnValue = ret;
    });
